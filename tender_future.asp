...
call WhereTable("identity.png","即将开始的竞价项目")
...
sql="select * from tender where ifdel=0 and ifzu=1 and statue=0"
sql=sql&" and tendergrade<="&companyinfo_grade_order
'sql=sql&" and djh in (select djh from competitive where companyid="&session("user_id")&")"
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><input type='button' value='收藏'<%if instr(rs("focusman"),","&session("user_id")&",")>0 then response.write " disabled"%> onClick="location.href='tender_future_focus.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=tender_future'" class='form_submit'></td>
	</tr>
