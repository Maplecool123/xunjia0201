...
call WhereTable("identity.png","我的收藏")
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><input type='button' value='取消' onClick="location.href='tender_future_nofocus.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>'" class='form_submit'></td>
		<td class="grid_cell"><input type='button' value='竞价'<%if shownamestrnull("id","competitive","djh",rs("djh")," and companyid="&session("user_id")&"")<>"" then response.write " disabled"%> onClick="location.href='tender_competitive_add.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=tender_mycollection'" class='form_submit'></td>
	</tr>
