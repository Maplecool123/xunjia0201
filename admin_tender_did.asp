...
call WhereTable("filefind.png","已完成的竞价")
...
<tr class="grid_odd" align="center">
	...
	<td class="grid_cell"><a href="#" onClick="doCheckDetail('admin_tender_did_printall.asp?sql=<%=Filtersql(sqlz)%>','1024','768')"><img src="images/print.gif" border="0" title="点击打印查询记录"></a>
	&nbsp;&nbsp;<a href="admin_tender_did_excel.asp?sql=<%=Filtersql(sqlz)%>" target="_blank"><img src="images/excel.jpg" border="0" title="点击导出项目记录"></a>
	&nbsp;&nbsp;<a href="admin_tender_did_excelall.asp?sql=<%=Filtersql(sqlz)%>" target="_blank"><img src="images/excel.jpg" border="0" title="点击导出物资记录"></a></td>
</tr>
</form>
</table>
<form name='form1'>
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><input type='button' value='打印' onClick="doCheckDetail('admin_tender_print_main.asp?id=<%=rs("id")%>','1024','768')" class='form_button'></td>
		<%if session("iflogin")-1=0 or session("iflogin")-99=0 then%>
		<td class="grid_cell">
		<%if rs("addman")-session("user_id")=0 then%>
			<input type='button' value='取消' onClick="if (confirm('你确定要取消<%=rs("tendername")%>竞价？\n\n取消后将划入“删除竞价”栏目')){location.href='admin_tender_did_cancel.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=admin_tender_did';}" class='form_submit'>
		<%else
			response.write "----"
		end if%></td>
		<%end if%>
