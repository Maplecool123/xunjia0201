...
call WhereTable("filefind.png","等待定标")
...
<tr class="grid_odd" align="center">
	...
	</select><select name="checkgrade" class='form_combo_normal' onChange="form2.submit();">
	...
	<option value="0"<%if nowcheckstatue="0" then%> selected="selected"<%end if%>>未打印</option>
	<option value="1"<%if nowcheckstatue="1" then%> selected="selected"<%end if%>>已打印</option>
	</select>
...
sql="select * from tender where ifdel=0 and ifzu=1 and statue=2 and djh not in (select djh from competitive where (statue=1 or statue=2) and ifzu=1)"
...
if nowcheckstatue<>"" then
	sql=sql&" and ifprint="&nowcheckstatue&""
end if
if nowkeyword1<>"" then
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><%response.write "<font color='#0000FF'><strong>"&rs("minmoney")&"</strong></font> / <font color='#FF0000'><strong>"&getmaxcompetitiveprice(rs("djh"),"")&"</strong></font>"%></td>
		<td class="grid_cell"><input type='button' value='查看' onClick="doCheckDetail('admin_tender_main_more.asp?id=<%=rs("id")%>',<%=Modalwidth%>,<%=Modalheight%>)" class='form_button'></td>
		<%if rs("ifprint")-1=0 then
			bvalue="已打印"
		else
			bvalue="打印"
		end if%>
		<%if session("iflogin")-1=0 or session("iflogin")-99=0 then%>
		<td class="grid_cell"><input type='button' value='<%=bvalue%>' onClick="doCheckDetail('admin_tender_print_main.asp?id=<%=rs("id")%>','1024','768')" class='form_button'></td>
		<td class="grid_cell"><%if rs("addman")-session("user_id")=0 then%>
		<input type='button' value='定标' onClick="location.href='admin_tender_decide_add.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=admin_tender_decide'" class='form_submit'>
		<%else
			response.write "----"
		end if%></td>
		<%end if%>
