...
%>
<head><title><%=sysname%> - <%=sysuser%>欢迎您！</title>
...
<tr>
	<td height="145" background="IMAGES/LoginBottom.gif" valign="top">
		...
		<tr>
			...
			<td>
				...
				<form name="loginform" method="post" action="index.asp?action=login" onSubmit="return checkform();">
				<tr>
					...
					<input type="reset" name="reset" value=" 重 填 " class="bdlogin">
					<%if canreg-1=0 then%>
					<input type="button" name="reg" value=" 注 册 " class="bdlogin" onClick="window.location.href='company_reg_first.asp'">
					<%end if%>
					&nbsp;
					<font color="#FFFFFF">点击下载：</font><a href="downfile/用户使用说明书.pdf" target="_blank"><font color="#FFFFFF">用户使用说明书</font></a></td>
				</tr>
				</form>
				</table>
			</td>
...
<TABLE height="30" cellSpacing="0" cellPadding="0" width="761" align="center" bgColor="#ff6600" border="0">
<TBODY>
<TR>
...
<%
	...
	sql="select * from company where (companyname='"&nowusername&"' or companyrealname='"&nowusername&"') and pwd='"&md5(nowpwd)&"'"
	...
	if rs.eof then
		...
		session("loginas")="企业"
		session("userrealname")=rs("companyrealname")
		session("username")=rs("companyname")
		session("user_id")=rs("id")
		session("iflogin")=0
		session("statue")=rs("statue")
