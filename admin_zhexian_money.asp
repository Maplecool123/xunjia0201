...
call WhereTable("login_shop.png","折线分析图")
...
total(0,1)="#FF0000,1.5,2,2,总计"'参数1线条的颜色，参数2线条的宽度，参数3线条的类型，参数4转折点的类型,参数5线条名称
total(0,2)="#0000FF,1.5,1,3,材料"
total(0,3)="#004D00,1.5,1,3,设备"
total(0,4)="#FFCC33,1.5,1,3,综合"
...
for i=1 to allno
	...
	if isSqldatabase=0 then
		sqltime=" and gettime between #"&getmonthstartdate(startyear,startmonth)&"# and #"&getmonthenddate(startyear,startmonth)+1&"#"
	else
		sqltime=" and gettime between '"&getmonthstartdate(startyear,startmonth)&"' and '"&getmonthenddate(startyear,startmonth)+1&"'"
	end if
	total(i,2)=getshuliang1("spotprice","competitive","(statue=1 or statue=2) and ifzu=1 and djh in (select djh from tender where tenderclass='材料')"&sqltime)
	total(i,3)=getshuliang1("spotprice","competitive","(statue=1 or statue=2) and ifzu=1 and djh in (select djh from tender where tenderclass='设备')"&sqltime)
	total(i,4)=getshuliang1("spotprice","competitive","(statue=1 or statue=2) and ifzu=1 and djh in (select djh from tender where tenderclass='综合')"&sqltime)
...
<tr class='grid_header'>
	<td colspan="4"><strong>项目中标总额：<%=allzhaobiao1%> 元&nbsp;&nbsp;&nbsp;&nbsp;材料类：<%=allzhaobiao2%> 元&nbsp;&nbsp;&nbsp;&nbsp;设备类：<%=allzhaobiao3%> 元&nbsp;&nbsp;&nbsp;&nbsp;综合类：<%=allzhaobiao4%> 元</strong></td>
</tr>
</form>
</table>
<%call table2(total,100,150,800,380,4)%>
<%else
...
response.write "<v:line id='_x0000_s1027' alt='' style='position:absolute;left:0;text-align:left;top:0;flip:y;z-index:-1' from='"&table_x+left_width&"px,"&table_y&"px' to='"&table_x+left_width&"px,"&table_y+all_height&"px'><v:stroke endarrow='block'/></v:line>"
	...
	response.write "<v:textbox inset='0px,0px,0px,0px'><table cellspacing='3' cellpadding='0' width='100%' height='100%'><tr><td align='center'>竞价数</td></tr></table></v:textbox></v:shape>"
	...
	response.write "<v:textbox inset='0px,0px,0px,0px'><table cellspacing='3' cellpadding='0' width='100%' height='100%'><tr><td align='left'>月份</td></tr></table></v:textbox></v:shape>"
