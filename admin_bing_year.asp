...
call WhereTable("yast_partitioner.png","竞价数量饼状分析图")
...
if isSqldatabase=0 then
	sqltime=" and startdate between #"&getmonthstartdate(nowstartyear,nowstartmonth)&"# and #"&getmonthenddate(nowendyear,nowendmonth)&"#"
else
	sqltime=" and startdate between '"&getmonthstartdate(nowstartyear,nowstartmonth)&"' and '"&getmonthenddate(nowendyear,nowendmonth)&"'"
end if

total(1,1)=gettotalitem("tender","tenderclass","材料"," and ifzu=1 and ifdel=0"&sqltime)
total(2,1)=gettotalitem("tender","tenderclass","设备"," and ifzu=1 and ifdel=0"&sqltime)
total(3,1)=gettotalitem("tender","tenderclass","综合"," and ifzu=1 and ifdel=0"&sqltime)
total(1,2)="材料"
total(2,2)="设备"
total(3,2)="综合"
...
<%
call table2(total,120,120,800,420,"竞价数量三维饼状图","项")
'参数含义(数组，横坐标，纵坐标，图表的宽度，图表的高度,图表标题,单位)%>
<%else
