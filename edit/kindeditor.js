...
KE.setting = {
	...
	statusbarHeight : 11,
	items : [
		'fullscreen', 'print', '|', 'undo', 'redo', '|', 'cut', 'copy', 'paste',
...
KE.lang = {
	...
	invalidAccessory : "请输入有效的URL地址。\n只允许doc,docx,xls,xlsx,pdf,txt,ppt,rar,zip格式。",
		...
		if (type == 2) {
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						//document.getElementById("include_file").value+="|"+data.url;
...
KE.plugin['accessory'] = {
    ...
    check : function(id) {
        ...
        if(url.match(/\.(doc|xls|ppt|pdf|txt|rar|zip|docx|xlsx|pptx)$/i) == null) {
    ...
    insert : function(id, url, title) {
        var html = '<img src="'+KE.g[id].skinsPath+'fujian.gif"/>&nbsp;';
		html += '<a href="' + url + '" >';
        if (title) html+= title;
        html += '</a>';
		//document.getElementById("include_file").value+="|"+url;
        KE.util.insertHtml(id, html);
        KE.layout.hide(id);
        KE.util.focus(id);
    }
