...
call WhereTable("filefind.png","尚未开始的竞价项目")
...
if request("action")="delall" then
	...
	for i=0 to UBound(id)
		set rs_t=conn.execute("select * from tender where id="&id(i)&"")
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<%if rs("addman")-session("user_id")=0 then%>
			<input type='button' value='修改' onClick="location.href='admin_tender_future_edit.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>'" class='form_submit'>
		<%else
			response.write "----"
		end if%></td>
		<td class="grid_cell">
