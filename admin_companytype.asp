...
call WhereTable("todo.png","企业类型管理")
...
if request("action")="delall" then
	...
	for i=0 to UBound(id)
		conn.execute("delete from [companytype] where id="&id(i)&"")
	next
...
<tr class="grid_odd" align="center">
	...
	<td class="grid_cell"><input type='button' value='新增' onClick="location.href='admin_companytype_add.asp'" class='form_submit'></td>
...
sql="select * from companytype where id<>0"
if nowcheckstyle="mohu" then
	if nowkeyword1<>"" then
		sql=sql&" and companytypename like '%"&nowkeyword1&"%'"
	end if
elseif nowcheckstyle="jing" then
	if nowkeyword1<>"" then
		sql=sql&" and companytypename = '"&nowkeyword1&"'"
	end if
end if
sql=sql&" order by companytypeorder"%>
<form name='form1'>
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><%=rs("companytypename")%></td>
		<td class="grid_cell"><%=rs("companytypeorder")%></td>
		<td class="grid_cell"><input type='button' value='修改' onClick="location.href='admin_companytype_edit.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>'" class='form_submit'></td>
