...
call WhereTable("identity.png","推荐竞价！")
...
sql=sql&" and djh not in (select djh from competitive where companyid="&session("user_id")&" and ifzu=1)"
...
if not rs.eof then
	...
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><input type='button' value='查看'  onClick="doCheckDetail('tender_main.asp?id=<%=rs("id")%>',<%=Modalwidth%>,<%=Modalheight%>)" class='form_button'></td>
		<td class="grid_cell"><input type='button' value='收藏'<%if instr(rs("focusman"),","&session("user_id")&",")>0 then response.write " disabled"%> onClick="location.href='tender_future_focus.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=tender_recommend'" class='form_submit'></td>
		<td class="grid_cell"><input type='button' value='竞价' onClick="location.href='tender_competitive_add.asp?id=<%=rs("id")%>&<%=gourl%>&page=<%=currentpage%>&fromurl=tender_recommend'" class='form_submit'></td>
	</tr>
