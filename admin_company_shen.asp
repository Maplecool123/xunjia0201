...
call WhereTable("identity.png","供应商审核")
...
sql="select * from company where id<>0 and statue=0"
if session("iflogin")-2<>0 then
	sql=sql&" and (ifshen2=0 or ifshen2 is null) and (ifshen1=0 or ifshen1 is null)"
end if
...
if not rs.eof then
  ...
  <tr class=<%=thisClass%> align="center">
    ...
    <td class="grid_cell"><input name="button" type='button' class='form_button'  onClick="doCheckDetail('company_main.asp?id=<%=rs("id")%>',<%=Modalwidth%>,<%=Modalheight%>)" value='查看'></td>
    <%if session("iflogin")-1=0 or session("iflogin")-2=0 or session("iflogin")-99=0 then%>
    <td class="grid_cell">
	<%if checkifnum(rs("shenman1"))-checkifnum(session("user_id"))=0 or checkifnum(rs("shenman2"))-checkifnum(session("user_id"))=0 then
		response.write "您已审核"
	else
		if (rs("ifshen1")>0 and session("iflogin")-2=0) or rs("ifshen1")=0 or isnull(rs("ifshen1")) then
			response.write "<input name='button' type='button' class='form_submit' onClick='location.href=""admin_company_shen_edit.asp?id="&rs("id")&"&"&gourl&"&page="&currentpage&"""' value='审核'>"
		else
			response.write "没有权限"
		end if
	end if%></td>
  ...
  <%
	...
	%>
  <tr class='grid_header'>
    <td colspan="20"><table class='list_table' width='100%' cellpadding='0' cellspacing='1' border='0'>
      <tr class="list_command">
        <td align="right"><%showdel="no"%>
