...
call WhereTable("identity.png","中标项目")
...
<tr class="grid_odd" align="center">
	...
	<option value="1"<%if nowcheckstatue="1" then%> selected="selected"<%end if%>>未确认</option>
	<option value="2"<%if nowcheckstatue="2" then%> selected="selected"<%end if%>>已确认</option>
	</select>
...
sql="select * from tender where ifdel=0 and ifzu=1 and statue=2"
'sql=sql&" and pretotalmoney>="&companyinfo_grade_price1&" and pretotalmoney<="&companyinfo_grade_price2&""
sql=sql&" and djh in (select djh from competitive where companyid="&session("user_id")&" and (statue=1 or statue=2) and ifzu=1)"
...
if nowcheckstatue<>"" then
	sql=sql&" and djh in (select djh from competitive where statue="&nowcheckstatue&" and companyid="&session("user_id")&")"
end if
if nowkeyword1<>"" then
...
sql=sql&" order by statue,addtime desc"%>
...
if not rs.eof then
	...
	needconfirm="yes"
	set rsc=conn.execute("select * from competitive where companyid="&session("user_id")&" and djh='"&rs("djh")&"'")
	if not rsc.eof then
		if rsc("statue")-2=0 then needconfirm="no"
	end if
	rsc.close
	%>
	<tr class=<%=thisClass%> align="center">
		...
		<td class="grid_cell"><input type='button' value='确认'<%if needconfirm="no" then response.write " disabled"%> onClick="location.href='tender_get_confirm.asp?djh=<%=rs("djh")%>&<%=gourl%>&page=<%=currentpage%>'" class='form_submit'></td>
	</tr>
